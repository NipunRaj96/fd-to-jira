...
load_dotenv()
# Suppress SSL warnings explicitly
...
def rate_limited_request(url, auth=None, **kwargs):
    ...
    try:
        # Ensure SSL verify is disabled for this request too
        kwargs.setdefault('verify', False)
        response = session.get(url, auth=auth, headers=HEADERS, timeout=30, **kwargs)
...
def fetch_conversations(ticket_id):
    ...
    try:
        ...
        for convo in response.json():
            # Extract ALL available conversation fields first
            conversation_data = {
                "id": convo.get("id"),
