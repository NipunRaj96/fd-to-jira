...
# Conversation Attachments Functions
CONVERSATION_ATTACHMENTS_FIELD = "Conversation Attachments"


def claim_next_null_conversation_attachments() -> int:
    """Find a record where Conversation Attachments == None, set to "I" and return its Ticket_ID.
    Returns -1 if none available.
    """
    _ensure_paths()
    with FileLock(LOCK_FILE_PATH):
...
def finalize_in_progress_conversations(evaluate_status_fn) -> int:
    """Resolve lingering "I" statuses for conversation attachments using the provided evaluator.
