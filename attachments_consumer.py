        ...
        for att in attachments_list:
            ...
            if dest.exists():
                ...
                try:
                    ...
                    logging.warning(f"[attachments] Failed to track existing file for {att_id}: {e}")
                continue
            ...
            if ok:
                ...
                try:
                    ...
                    logging.warning(f"[attachments] Failed to track download for {att_id}: {e}")
...
def worker_loop(stop_flag: List[bool]) -> None:
    while not stop_flag[0]:
        ...
        if tid == -1:
            # Keep running indefinitely - no termination
            time.sleep(0.5)
            continue
